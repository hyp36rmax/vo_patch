...
[docs/NOTES.md](NOTES.md#installing-from-a-disc-image).
...
The offsets and internals of every patch are in [NOTES.md](NOTES.md).
...
For controller profiles, mappings, player ownership and Custom remapping, see
[Controller Expansion](../README.md#controller-profiles). English retail and
Japanese rerelease use explicit per-player controller selection; USA OEM and
Japanese original retain upstream connected-slot ordering.
...
gamepad, but each sees only its own inputs - the pad inputs on one page,
